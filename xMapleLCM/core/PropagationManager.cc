...
namespace Minisat {
    ...
    PropagationManager::PropagationManager(Solver* s)
        : watches_bin(s->ca)
        , watches(s->ca)
...
#ifdef TESTING
    inline void  PropagationManager::set_value(Var x, lbool v, int l) {
    ...
    inline lbool PropagationManager::value(Var x) const { auto it = test_value.find(x); return (it == test_value.end()) ? (l_Undef) : (it->second.first ); }
    inline lbool PropagationManager::value(Lit p) const { return value(var(p)) ^ sign(p); }
#else
    inline lbool PropagationManager::value(Var x) const { return solver->value(x); }
    inline lbool PropagationManager::value(Lit p) const { return solver->value(p); }
#endif

    void PropagationManager::relocAll(ClauseAllocator& to) {
        // Clean watchers
    ...
    inline CRef PropagationManager::propagate_single(Lit p) {
    ...
    CRef PropagationManager::propagate() {
        CRef    confl     = CRef_Undef;
        int     num_props = 0;
        Lit     p         = lit_Undef;
        watches.cleanAll();
    ...
    CRef PropagationManager::simplePropagate() {
        CRef    confl = CRef_Undef;
        int     num_props = 0;
        watches.cleanAll();
        watches_bin.cleanAll();
    ...
    void PropagationManager::enforceWatcherInvariant(CRef cr, int i_undef, int i_max) {
