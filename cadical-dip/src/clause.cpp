...
void Internal::mark_removed (Clause *c, int except) {
  ...
  assert (!c->redundant or c->extended);
  for (const auto &lit : *c)
    if (lit != except)
      mark_removed (lit);
}
...
Clause *Internal::new_learned_redundant_clause (int glue) {
  ...
  for (size_t i = 2; i < clause.size (); i++)
    assert (val(clause[0]) == 0 or var (clause[0]).level >= var (clause[i]).level),
...
// Add definition clause during extended variable addition
Clause *Internal::new_extended_definition_clause ( ) {
  assert(clause.size() == 2 or clause.size() == 3);
  Clause *res = new_clause (false,INT_MAX);
  res->extended = true;
  if (proof) 
    proof->add_derived_clause(res,lrat_chain);
  assert(watching());
