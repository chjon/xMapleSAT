...
inline bool Internal::bump_also_reason_literal (int lit) {
  assert (lit);
  //assert (val (lit) < 0);  when learning DIP -> conflict, the DIP varialbe might be new and hence UNDEF
  Flags &f = flags (lit);
  if (f.seen)
    return false;
  const Var &v = var (lit);
...
inline void Internal::bump_also_reason_literals (int lit, int limit) {
  ...
  assert (limit > 0);
  const Var &v = var (lit);
  //assert (val (lit)); // ALBERT: when learning DIP -> conflict, the DIP varialbe might be new and hence UNDEF
  if (val(lit) >= 0) return;
...
void Internal::clear_analyzed_literals () {
  ...
  for (const auto &lit : analyzed) {
    Flags &f = flags (lit);
    // assert (f.seen); // ALBERT removed it
    f.seen = false;
    assert (!f.keep);
    assert (!f.poison);
    assert (!f.removable);
...
bool Internal::check_all_literals_cleared () {
  for (auto v : vars) 
    if (flags(v).seen) return false;
  return true;
}
  
void Internal::clear_analyzed_levels () {
  LOG ("clearing %zd analyzed levels", levels.size ());
  // cout << "Clearing levels control size is " << control.size() << " : ";
  // for (const auto &l : levels) cout << l << " ";
  // cout << endl;
...
bool Internal::check_all_levels_cleared () {
  for (int  l = 0; l < level; ++l) {
    if (not control[l].is_reset()) {
      cout << "Problem with level " <<  l << endl;
      return false;
    }
  }
  return true;
...
Clause *Internal::new_driving_clause (const int glue, int &jump) {
  ...
  if (!size) {
    ...
    //
    MSORT (opts.radixsortlim, clause.begin () + 1, clause.end (),
...
void Internal::analyze_1UIP () {
  ...
  //   cout << endl;
  //   cout << "ClauseDIP: ";
  //   for (int x : clauseDIP) cout << x << " ";
  //   cout << endl;
  // }
  // assert(clause == clauseDIP);
...
void Internal::iterate () {
  ...
  // cout << endl;
  int uip = -main_clause[0]; // try_dip_analysis guarantees that
  ...
  //
  if (size > 1) {
    if (opts.shrink and false) // ALBERT CHANGE (not yet ready for minimization!!!!)
      shrink_and_minimize_clause ();
    else if (opts.minimize and false) // ALBERT CHANGE (not yet ready for minimization!!!!)
      minimize_clause ();
    ...
    // Update decision heuristics. Note that clause should the the learned lemma and analyzed should
    // contain the literals that have been analyzed
    //
    analyzed = analyzed_lits;
    if (opts.bump) 
      bump_variables ();
  ...
  Clause *driving_clause = new_driving_clause (glue, jump); // this sorts so that watch is ok
                                                            // it assumes lit at pos 0 is correctly placed (watched)
  //cout << "Glue " << glue << " jump " << jump << endl;
  UPDATE_AVERAGE (averages.current.jump, jump);

  // cout << "Driving clause is: " ;
  // if (size == 1) cout << clause[0] << endl;
  // else {
  //   for (int x : *driving_clause) cout << x << " ";
  //   cout << endl;
  // }
  
  int new_level = determine_actual_backtrack_level (jump);
  //  cout << "new_level " << new_level << endl;
  UPDATE_AVERAGE (averages.current.level, new_level);
  backtrack (new_level);

  //cout << "BJ to " << new_level << endl;
  assert(check_all_literals_cleared());
  ...
  //
  if (uip) {
    //cout << "We are assigning " << -uip << endl;
    search_assign_driving (-uip, driving_clause);
    //cout << "assigned" << endl;
  } else
    learn_empty_clause ();
  ...
  //cout << "Conflict " << stats.conflicts << " OK" << endl;
}
