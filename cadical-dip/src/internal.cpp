...
namespace CaDiCaL {
  ...
  void Internal::enlarge (int new_max_var) {
    ...
    enlarge_zero (is_extended_actively_deleted, new_vsize);
    vsize = new_vsize;
  }

  void Internal::enlarge_er (int new_max_var) {
    size_t new_vsize = vsize ? 2 * vsize : 1 + (size_t) new_max_var;
    while (new_vsize <= (size_t) new_max_var)
      new_vsize *= 2;
    LOG ("enlarge internal size from %zd to new size %zd", vsize, new_vsize);
  ...
  void Internal::init_vars_er (int new_max_var) {
    if (new_max_var <= max_var)
      return;
  ...
  int Internal::local_search () {
    ...
    cout << "Local search deactivated due to assertions" << endl;
    return 0;
