...
void Internal::mark_satisfied_clauses_as_garbage () {
  ...
  for (const auto &c : clauses) {
    ...
    if (tmp > 0){
      // if (c->extended) {
      // 	// cout << "Satisfied definition at DL 0" << endl;
      // 	// for (int l : *c) cout << l << " ";
      // 	// cout << endl;
      // }
      mark_garbage (c);
      ...
      // if (c->extended) {
      // 	cout << "Some literal falsified in definition at DL 0" << endl;
      // 	for (int l : *c) cout << l << "(val " << int(val(l)) << ", lev " << var(l).level << ") ";
      // 	cout << endl;
      // }
...
inline void Internal::flush_watches (int lit, Watches &saved) {
  ...
  for (i = j; i != end; i++) {
    ...
    if (c->literals[!new_blit_pos] != lit) {
      cout << "Clause: ";
      for (int x : *c) cout << x << " ";
      cout << endl;
