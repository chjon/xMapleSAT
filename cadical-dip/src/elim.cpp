...
void Internal::elim (bool update_limits) {
  ...
  // cout << "ELIM" << endl;
  // if (max_var >= 509) {
  //   Clause* cl = extended_var_2_definitions[509][0];
  //   for (auto l : *cl) cout << l << " ";
  //   cout << "deleted " << cl->garbage << endl;
  // }
  stats.elimphases++;
  PHASE ("elim-phase", stats.elimphases,
  ...
  while (!unsat && !phase_complete && !terminated_asynchronously ()) {
    ...
    cout << "Before elim round" << endl;
    Clause *tmp = find_binary_extended_clause_in_DB(-96,388);
    if (tmp == NULL) {cout << "Not found" << endl;exit(1);}
    else cout << "Found (deleted " << tmp->garbage << endl;
    ...
    cout << "Done" << endl;
    tmp = find_binary_extended_clause_in_DB(-96,388);
    if (tmp == NULL) {cout << "Not found" << endl;exit(1);}
    else cout << "Found (deleted " << tmp->garbage << ")" << endl;
