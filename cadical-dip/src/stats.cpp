...
void Stats::print (Internal *internal) {
  ...
  if (all || stats.conflicts) {
    ...
    PRT ("dip_conflicts:   %15" PRId64 "   %10.2f %% of all conflicts",
	 stats.dip_conflicts, percent(stats.dip_conflicts, stats.conflicts) );
    PRT ("dip_exists:      %15" PRId64 "   %10.2f %% of all conflicts",
	 stats.dip_exists, percent(stats.dip_exists, stats.conflicts) );
    PRT ("dip_dangerous:   %15" PRId64 "   %10.2f %% of all conflicts",
	 stats.dip_dangerous, percent(stats.dip_dangerous, stats.conflicts) );
    PRT ("num_ext_vars:    %15" PRId64,
         internal->max_var - internal->orig_max_var);
  ...
  if (all || stats.decisions) {
    ...
    PRT ("decisions_ext    %15" PRId64 "   %10.2f %% of all decisions",
	 stats.dip_decision, percent(stats.dip_decision, stats.decisions) );   
